...
그림에서 볼수 있듯이 case:1의 경우 2번 블록에 거래 내용이 저장되고 다음블록이 생성되지 않았다. 이때 악의적인 사용자들은 2번블록에 저장된 거래 내역을 파괴하기 위해 2번블록, 3번블록을 빠르게 만들어 정상적인 사용자와 연결 하면 된다. 하지만 case:2처럼 2번블록에 거래 내용이 저장되고 3번 블록이 생성된다면 악의적인 사용자들은 2번, 3번, 4번 블록까지 블록을 만든 후 정상적인 사용자와 연결을 해야한다. 

즉 내 거래 내용이 저장된 블록 이후로 더 많은 블록이 생산된다면 내 거래 내용을 공격하기 위해서는 공격자가 더 많은 블록을 더 빠르게 만들어야 한다.

비트코인에서는 6개의 블록이 더 생산되면 공격당할 확률은 매우 적다고 이야기 한다.
