...
블록체인 기술을 사용한 비트코인에서는 언제나 가장 긴 블록이 정식 블록이라 생각하고 채택한다.

그래서 정상적인 유저가 가진 해시 연산력보다 큰 연산력을 가지게 된다면 정상적인 유저들보다 더 빠르게 블록을 생성해 저장할수 있다.

이점을 공격자들은 이용해 거래가 담겨있는 블록을 조작한 후 정상적인 유저들보다 더 빠르게 만든다면 공격자들의 블록이 정식 블록으로 채택이 되고 블록체인 안에서 일어나는 거래들이 공격을 받게 된다.
